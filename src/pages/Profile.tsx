...
export default function Profile() {
  ...
  const gradeDisplay =
    typeof user.grade === 'string' && user.grade.trim()
      ? user.grade
      : '—';
  ...
  const fields = [
    ...
    { icon: BadgeCheck, label: 'Grade', value: gradeDisplay },
    { icon: Building2, label: 'Department', value: user.department || '—' },
    {
      icon: CalendarDays,
      label: 'Date of Joining',
      value: user.dateOfJoining ? formatDate(user.dateOfJoining) : '—',
    },
          ...
          <div className="flex flex-wrap items-start gap-4">
            ...
            <div className="min-w-0 flex-1 pt-1">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {gradeDisplay !== '—' && <Badge variant="teal">{gradeDisplay}</Badge>}
                {user.status === 'active' ? (
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className={`mt-0.5 truncate text-sm font-medium text-gray-900 ${f.mono ? 'font-mono' : ''}`}>
                  {f.value}
                </p>
              </div>
            </div>
          ...
          {balances.map((b) => (
            <div key={b.leaveType} className="rounded-xl bg-gray-50 p-4">
              ...
              <p className="mt-1 text-2xl font-semibold text-gray-900">
                {b.remaining}
                <span className="text-sm font-normal text-gray-400"> / {b.quota}</span>
              </p>
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-gray-200">
                <div
                  className="h-full rounded-full bg-blue-500"
                  style={{ width: `${b.quota > 0 ? (b.used / b.quota) * 100 : 0}%` }}
                />
              </div>
            ...
            >
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {formatDate(l.startDate)} → {formatDate(l.endDate)} · {l.daysUsedBeforeCancel ?? l.totalDaysRequested} day(s)
                </p>
              </div>
