...
import { AlertCircle, Pencil, Plus, Lock, Trash2 } from 'lucide-react';
...
export default function Policies() {
  ...
  const openEdit = (p: LeavePolicy) => {
    ...
    setForm({
      ...
      finalApprovalMode: p.finalApprovalMode || false,
      documentRequirement:
        p.documentRequirement ||
        (p.requiresDocumentUpload ? 'required' : 'optional'),
    });
    setValidationError('');
  };
  ...
  const handleSave = () => {
    if (!form.leaveTypeName.trim()) {
      setValidationError('Please enter a leave type name.');
      return;
    }

    if (!form.finalApprovalMode && form.approverIds.length === 0) {
      setValidationError('Please select at least one required approver.');
      return;
    }
    ...
    const policyPayload: LeavePolicy = {
      ...
      finalApprovalMode: form.finalApprovalMode,
      requiresDocumentUpload: form.documentRequirement === 'required',
      documentRequirement: form.documentRequirement,
      // Minimum notice has been removed from the UI, but the old value is
      // preserved for compatibility with the rest of the current project.
      minDaysNoticeRequired: editing?.minDaysNoticeRequired ?? 0,
      isPaid: form.isPaid,
    };
              ...
              <div className="mt-4 space-y-2.5 text-sm">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex max-w-[68%] flex-wrap justify-end gap-1">
                    {p.finalApprovalMode ? (
                      <>
                        <Badge variant="green">Final Manager</Badge>
                        <Badge variant="blue">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {p.documentRequirement === 'required' ? (
                    <Badge variant="orange">Required</Badge>
                  ) : p.documentRequirement === 'not_required' ? (
                    <Badge variant="gray">Not Required</Badge>
                  ) : (
                ...
                <div className="flex items-center justify-between">
                  ...
                  {p.isPaid ? (
                    ...
                    <Badge variant="gray">Unpaid</Badge>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4 text-left max-h-[60vh] overflow-y-auto px-1">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, leaveTypeName: e.target.value })
                }
                className={inputCls}
              />
            </div>
            ...
            <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4">
              <label className="flex cursor-pointer items-start gap-3">
                <input
                  type="checkbox"
                  checked={form.finalApprovalMode}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      finalApprovalMode: e.target.checked,
                    })
                  }
                ...
                <div>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-gray-500">
                    Enable this when the employee&apos;s assigned Manager should
                    make the final approve or reject decision. The request will
                    not continue to another approver.
                  </span>
                </div>
              </label>
            </div>
            ...
            {form.finalApprovalMode && (
              <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3">
                ...
                <p className="mt-1 text-xs leading-5 text-blue-700">
                  The leave request will automatically go to the Manager
                  assigned to that employee.
                </p>
                <p className="mt-1 text-xs leading-5 text-blue-600">
                  The assigned Manager&apos;s approve or reject decision will be
                  final.
                </p>
              </div>
            )}
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={() =>
                                setForm({
                                  ...form,
                                  approverIds: form.approverIds.filter(
                                    (aid) => aid !== id,
                                  ),
                                })
                              }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    documentRequirement: e.target.value as
                      | 'optional'
                      | 'required'
                      | 'not_required',
                  })
